...
class Noeud(Cliquable):
    ...
    def trouve_actif(self):
        if self.actif:
            ...
            raise NavigationError(f"Erreur : on a atteint {self} qui n'est pas actif, mais n'a pas de courant dénini !")
    ...
    def navigue(self,direction: Direction) -> Cliquable|Literal[False]:
        if self.actif: #On est à ce niveau
            ...
            raise NavigationError("Knot.navigue : On est à un niveau inférieur, mais on est monté jusqu'ici quand même avec self.courant qui vaut None")
        else:
    ...
    def navigue_in(self,direction: Direction) -> Cliquable|Literal[False]: # On est l'élément actif
        ...
        if direction == DirectionAff.IN:
            ...
            raise NavigationError(f"Knot.navigue_in : Direction inconnue : {direction}")

    def navigue_through(self,direction: Direction) -> Cliquable|Literal[False]: # On est sur le chemin de l'élément actif
        ...
        if direction == DirectionAff.IN:
            ...
            raise NavigationError(f"Knot.navigue_through : Direction inconnue : {direction}")

    def in_in(self) -> Cliquable|Literal[False]: # On veut aller plus profond
    ...
    def in_out(self) -> Cliquable|Literal[False]: # On veut ressortir
        """Navigue vers l'élément précédent dans la hiérarchie."""
        self.unset_actif()
        return False

    def in_previous(self) -> Cliquable|Literal[False]: # On veut aller au précédent de l'élément actuel
        """Navigue d'un enfant direct à un autre."""
        return False # On laisse faire l'élément parent

    def in_next(self) -> Cliquable|Literal[False]: # On veut aller au suivant de l'élément actuel
        """Navigue d'un enfant direct à un autre."""
        return False

    def in_left(self) -> Cliquable|Literal[False]: # On veut déplacer notre curseur (self.courant) vers la gauche
        """Navigue d'un enfant direct à un autre."""
        return False # On ne fait rien par défaut

    def in_right(self) -> Cliquable|Literal[False]: # On veut déplacer notre curseur (self.courant) vers la droite
        """Navigue d'un enfant direct à un autre."""
        return False # On ne fait rien par défaut

    def in_up(self) -> Cliquable|Literal[False]: # On veut déplacer notre curseur (self.courant) vers le haut
        """Navigue d'un enfant direct à un autre."""
        return False # On ne fait rien par défaut

    def in_down(self) -> Cliquable|Literal[False]: # On veut déplacer notre curseur (self.courant) vers le bas
        """Navigue d'un enfant direct à un autre."""
        return False # On ne fait rien par défaut

    def through_in(self) -> Cliquable|Literal[False]: # On veut aller plus profond, ça ne nous concerne pas
        """Navigue vers l'élément suivant dans la hiérarchie, à partir d'un enfant."""
        return False

    def through_out(self) -> Cliquable|Literal[False]: # On veut revenir ici
    ...
    def through_previous(self) -> Cliquable|Literal[False]: # On veut aller au précédent de notre élément actuel
        """Navigue d'un enfant à un autre."""
        return False # On ne fait rien par défaut

    def through_next(self) -> Cliquable|Literal[False]: # On veut aller au suivant de notre élément actuel
        """Navigue d'un enfant à un autre."""
        return False # On ne fait rien par défaut

    def through_left(self) -> Cliquable|Literal[False]: # On veut déplacer le curseur, ça ne nous concerne pas
        """Navigue d'un enfant à un autre."""
        return False

    def through_right(self) -> Cliquable|Literal[False]: # On veut déplacer le curseur, ça ne nous concerne pas
        """Navigue d'un enfant à un autre."""
        return False

    def through_up(self) -> Cliquable|Literal[False]: # On veut déplacer le curseur, ça ne nous concerne pas
        """Navigue d'un enfant à un autre."""
        return False

    def through_down(self) -> Cliquable|Literal[False]: # On veut déplacer le curseur, ça ne nous concerne pas
        """Navigue d'un enfant à un autre."""
        return False
